...
function Warrior:ClassInit()
	...
	self.CastSuccessHandlers[355] = self.Taunt
...
-- 	print("warrior taunt! " .. UnitGUID("targettarget"))
-- 	local targetThreat = ThreatLib:GetThreat(UnitGUID("targettarget"), target, "target", true)
-- 	local myThreat = ThreatLib:GetThreat(UnitGUID("player"), target, "target", true)

-- 	print("my threat: " .. myThreat .. " targettarget threat: " .. targetThreat)
...
function Warrior:TauntNextHook(timestamp, subEvent, hideCaster, sourceGUID, sourceName, sourceFlags, sourceRaidFlags, destGUID, destName, destFlags, destRaidFlags, spellID)
	if pendingTauntTarget and (subEvent ~= "SPELL_MISSED" and spellID ~= 355) then
