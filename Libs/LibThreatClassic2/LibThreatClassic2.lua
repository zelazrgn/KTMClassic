...
function ThreatLib:UpdatePlayerTarget()
	local unit = "player"
	lastTarget[unit] = UnitGUID("target") or lastTarget[unit]
	lastTargetName[unit] = UnitName("target") or lastTargetName[unit]
end
...
function ThreatLib:OnInitialize()
	...
	self:RegisterComm(self.ktm_prefix)
...
function ThreatLib:ThreatUpdated(source_unit, target_guid, threat)
	-- target_guid = ".ktm"
	-- print("ThreatLib:ThreatUpdated: " .. source_unit .. " " .. target_guid .. " " .. threat)
	self:ThreatUpdatedForUnit(source_unit, target_guid, threat)
	-- self:ThreatUpdatedForUnit(source_unit, ".ktm", threat)
	local t = GetTime()
...
function ThreatLib:ThreatUpdatedForUnit(unit_id, target_guid, threat)
	...
	-- print("ThreatUpdatedForUnit " .. unit_id .. " " .. target_guid .. " " .. threat)
...
ThreatLib.masterTarget = nil;

function ThreatLib:SetMasterTarget(mt)
	self.masterTarget = mt
	_callbacks:Fire("MasterTargetChanged")
	self:PublishThreat()
end

function ThreatLib:ClearThreat()
	local module = "Player-r"..MINOR
...
function ThreatLib.OnCommReceive:KTM_SET_MASTER_TARGET(sender, distribution, msg)
	...
	if strlen(newmt) > 0 then
		self:SetMasterTarget(newmt)
		print(format("KTM: The master target has been set to |cffffff00%s|r by |cffffff00%s|r.", self.masterTarget, sender))
	end
end
...
function ThreatLib.OnCommReceive:KTM_CLEAR(sender, distribution, msg)
	...
	if msg == "ear" then --
		...
		self:ClearThreat()
	elseif self.masterTarget ~= nil then
		print(format("KTM: The master target has been cleared by |cffffff00%s|r.", sender))
		ThreatLib:SetMasterTarget(nil)
	end
end

function ThreatLib.OnCommReceive:KTM_THREAT_UPDATE(sender, distribution, msg)
	if not msg then return end
	local threat = msg
	...
	if unitGUID then
		local target = "global"
		if self.masterTarget then
			target = "mt-"..self.masterTarget
		end
		self:ThreatUpdatedForUnit(unitGUID, target, tonumber(threat))
	end
end
...
function ThreatLib:_clearThreat(guid)
	...
	-- this should be covered by the publish
...
do
	...
	local function getThreatStringKTM(unit, module, force)
		local mod = ThreatLib:GetModule(module, true)
		if not mod or not mod:IsEnabled() then return nil, false end
		local nl = 0
		local uid = UnitGUID(unit) or mod.unitGUID
		if not uid then return nil, false end
		local tid = lastTarget[unit]
		if not tid then return nil, false end

		if ThreatLib.masterTarget and lastTargetName[unit] ~= ThreatLib.masterTarget then
			return nil, false
		end
		-- print("getThreatStringKTM: ".. lastTarget)
		-- t[1] = uid .. ":"
		local changed = false
		for k, v in pairs(mod.targetThreat) do
			if k == tid then
		...
		for k, v in pairs(lastPublishedThreat[unit]) do
			-- print("mod.lastPublishedThreat k: "..k.. " v: "..v)
			if mod.targetThreat[k] == nil then
				if k == tid and v ~= 0 then
					nl = nl + 1
					t[nl] = "0"
					changed = true
				end
	...
	function ThreatLib:PublishThreat(force)
		...
		local playerMsg = getThreatStringKTM("player", "Player-r"..MINOR, force)
		local petMsg = getThreatStringKTM("pet", "Pet-r"..MINOR, force)
		...
		if playerMsg then
			self:SendCommMessage(self.ktm_prefix, "t " .. playerMsg, self:GroupDistribution())
		end
...
function ThreatLib:KTMSetMasterTarget(targetName)
	...
	if targetName then
		...
		self:SendCommMessage(self.ktm_prefix, "cleartarget", self:GroupDistribution())
	end
	self:SetMasterTarget(targetName)
end
...
------------------------------------------------------------------------
function ThreatLib:GetThreat(player_guid, target_guid, target, d)
	if d then
			print("get threat: " .. player_guid)
	end
	...
	local threatsum = data[target_guid] or 0
	if self.masterTarget then
		if target then
		...
		if d then
			print("global: " .. (data['global'] or 0))
		end
		threatsum = threatsum + (data['global'] or 0)
	end
	-- print("ThreatLib:GetThreat: player: " .. player_guid .. " target: " .. target_guid .. " threat: " .. threatsum)
	return max(0, threatsum)
end
...
function ThreatLib:UnitDetailedThreatSituation(unit, target)
	...
	-- print("UnitDetailedThreatSituation" .. " source: " .. unit .. " target: " .. target)
	...
	threatValue = self:GetThreat(unitGUID, targetGUID, target) or 0
