...
LSM:Register("font", "NotoSans SemiCondensedBold", [[Interface\AddOns\KTMClassic\media\NotoSans-SemiCondensedBold.ttf]])
...
local function ThreatUpdated(event, unitGUID, targetGUID, threat)
	-- if UnitGUID(TC2.playerTarget) == targetGUID then
		-- print("threatclassic update", event, unitGUID, targetGUID, threat)
		CheckStatusDeferred()
	-- end
end
...
function TC2:PLAYER_LOGIN()
	...
	self.db = LibStub("AceDB-3.0"):New("KTMClassicDB", self.defaultConfig)
	-- check if per character settings still exist. If yes copy over to db
	if KTMC_Options then
		print("KTMClassic copying old config to new character profile.")
		if KTMC_Options.bar and KTMC_Options.bar.texture then
			-- remove old non LSM texture
			KTMC_Options.bar.texture = nil
		end
		self.db.profile = CopyLegacySettings(KTMC_Options, self.db.profile)
		KTMC_Options = nil
	end
	...
	if self.classic then
		...
		ThreatLib.RegisterCallback(self, "MasterTargetChanged", CheckStatusDeferred)
